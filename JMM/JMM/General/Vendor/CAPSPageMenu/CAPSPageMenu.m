...
- (void)setUpMenuItemView:(CGFloat)menuItemWidth menuScrollViewHeight:(CGFloat)menuScrollViewHeight indicatorHeight:(CGFloat)indicatorHeight separatorPercentageHeight:(CGFloat)separatorPercentageHeight separatorWidth:(CGFloat)separatorWidth separatorRoundEdges:(BOOL)separatorRoundEdges menuItemSeparatorColor:(UIColor *)menuItemSeparatorColor
{
    ...
    _menuItemSeparator = [[UIView alloc] initWithFrame:CGRectMake(menuItemWidth - separatorWidth, floor(menuScrollViewHeight * ((1.0 - separatorPercentageHeight) / 2.0)), separatorWidth, floor(menuScrollViewHeight * separatorPercentageHeight))];
    
//    if (separatorRoundEdges) {
//        _menuItemSeparator.layer.cornerRadius = _menuItemSeparator.frame.size.width / 2;
//    }
    _menuItemSeparator.hidden = separatorRoundEdges;
...
NSString * const CAPSPageMenuOptionSelectionIndicatorBottomMargin       = @"selectionIndicatorBottomMargin";
...
NSString * const CAPSPageMenuOptionNeedSetStartMargin                   = @"needSetStartMargin";
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    if (options) {
        for (NSString *key in options) {
            ...
            }else if ([key isEqualToString:CAPSPageMenuOptionNeedSetStartMargin]) {
                _needSetStartMargin = [options[key] boolValue];
            }else if ([key isEqualToString:CAPSPageMenuOptionSelectionIndicatorBottomMargin]){
                _selectionIndicatorBottomMargin = [options[key] floatValue];
            }
        }
...
- (void)initValues
{
    ...
    _scrollAnimationDurationOnMenuItemTap = 250;
    ...
    _selectionIndicatorColor      = [UIColor orangeColor];
    ...
    _menuItemSeparatorRoundEdges       = YES;
...
//    UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapGesTap:)];
//    tapGes.delegate = self;
//    [_controllerScrollView addGestureRecognizer:tapGes];
...
- (void)configureUserInterface
{
    ...
    if (_useMenuLikeSegmentedControl) {
        ...
        if (_needSetStartMargin) {
            _menuScrollView.contentSize = CGSizeMake((_menuItemWidth + _menuMargin) * (CGFloat)_controllerArray.count + _menuMargin - (_menuMargin - 15.0f)*2, _menuHeight);
        }else{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            if (_needSetStartMargin) {
                if (index == 0) {
                    menuItemFrame = CGRectMake(15.0f, 0.0, _menuItemWidth, _menuHeight);
                }else{
                    menuItemFrame = CGRectMake(_totalMenuItemWidthIfDifferentWidths + _menuMargin + (_menuMargin * index) - (_menuMargin - 15.0f), 0.0, _menuItemWidth, _menuHeight);
                }
            }else{
        ...
        if (_useMenuLikeSegmentedControl) {
            [menuItemView setUpMenuItemView:(CGFloat)self.view.frame.size.width / (CGFloat)_controllerArray.count menuScrollViewHeight:_menuHeight indicatorHeight:_selectionIndicatorHeight separatorPercentageHeight:_menuItemSeparatorPercentageHeight separatorWidth:_menuItemSeparatorWidth separatorRoundEdges:_controllerArray.count - index - 1 == 0 ? YES :_menuItemSeparatorRoundEdges menuItemSeparatorColor:_menuItemSeparatorColor];
            
        } else {
            [menuItemView setUpMenuItemView:_menuItemWidth menuScrollViewHeight:_menuHeight indicatorHeight:_selectionIndicatorHeight separatorPercentageHeight:_menuItemSeparatorPercentageHeight separatorWidth:_menuItemSeparatorWidth separatorRoundEdges:_controllerArray.count - index - 1 == 0 ? YES :_menuItemSeparatorRoundEdges menuItemSeparatorColor:_menuItemSeparatorColor];
        }
        ...
        if (_useMenuLikeSegmentedControl) {
            if ((NSInteger)index < _controllerArray.count - 1) {
                ...
                menuItemView.menuItemSeparator.hidden = YES;
            }
        }
    ...
    if (_menuItemWidthBasedOnTitleTextWidth) {
        if (_needSetStartMargin) {
            _menuScrollView.contentSize = CGSizeMake((_totalMenuItemWidthIfDifferentWidths + _menuMargin) + (CGFloat)_controllerArray.count * _menuMargin - (_menuMargin - 15.0f)*2, _menuHeight);
        }else{
    ...
    if (_useMenuLikeSegmentedControl) {
        selectionIndicatorFrame = CGRectMake(0.0, _menuHeight - _selectionIndicatorHeight - _selectionIndicatorBottomMargin,  _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : self.view.frame.size.width / (CGFloat)_controllerArray.count, _selectionIndicatorHeight);
    } else if (_menuItemWidthBasedOnTitleTextWidth) {
        if (_needSetStartMargin) {
            selectionIndicatorFrame = CGRectMake(15.0f, _menuHeight - _selectionIndicatorHeight- _selectionIndicatorBottomMargin, _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : [_mutableMenuItemWidths[0] floatValue], _selectionIndicatorHeight);
        }else{
            selectionIndicatorFrame = CGRectMake(_menuMargin, _menuHeight - _selectionIndicatorHeight - _selectionIndicatorBottomMargin, _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : [_mutableMenuItemWidths[0] floatValue], _selectionIndicatorHeight);
        }
        
    } else {
        if (_centerMenuItems) {
            selectionIndicatorFrame = CGRectMake(_startingMenuMargin + _menuMargin, _menuHeight - _selectionIndicatorHeight - _selectionIndicatorBottomMargin, _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : _menuItemWidth, _selectionIndicatorHeight);
        } else {
            selectionIndicatorFrame = CGRectMake(_menuMargin, _menuHeight - _selectionIndicatorHeight - _selectionIndicatorBottomMargin, _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : _menuItemWidth, _selectionIndicatorHeight);
        }
    }
    
    _selectionIndicatorView = [[UIView alloc] initWithFrame:selectionIndicatorFrame];
    if (_selectionIndicatorView > 0) {
        MenuItemView *menuItemView = _mutableMenuItems[_currentPageIndex];
        _selectionIndicatorView.centerX = menuItemView.centerX;
    }else{
        _selectionIndicatorView.frame = selectionIndicatorFrame;
    }
    ...
    if (!_didLayoutSubviewsAfterRotation) {
        if ([scrollView isEqual:_controllerScrollView]) {
            if (scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= ((CGFloat)(_controllerArray.count - 1) * self.view.frame.size.width)) {
                ...
                if([_delegate respondsToSelector:@selector(pageMenuScrollView:didScrollToLeftBoundary:)]){
                    [_delegate pageMenuScrollView:scrollView didScrollToLeftBoundary:NO];
                }
                ...
                // add by ZJ
                if(scrollView.contentOffset.x < 0.0){ // 左边界
                    if([_delegate respondsToSelector:@selector(pageMenuScrollView:didScrollToLeftBoundary:)]){
                        [_delegate pageMenuScrollView:scrollView didScrollToLeftBoundary:YES];
                    }
                }
...
- (void)moveSelectionIndicator:(NSInteger)pageIndex
{
    if (pageIndex >= 0 && pageIndex < _controllerArray.count) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            if (self.needSetStartMargin) {
                CGRect selectionIndicatorFrame = CGRectMake(selectionIndicatorX - (self.menuMargin -15.0f), self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth, self.selectionIndicatorView.frame.size.height);
                _selectionIndicatorView.frame = selectionIndicatorFrame;
                if (_selectionIndicatorView > 0) {
                    MenuItemView *menuItemView = _mutableMenuItems[_currentPageIndex];
                    _selectionIndicatorView.centerX = menuItemView.centerX;
...
//                 self.selectionIndicatorView.frame = CGRectMake(selectionIndicatorX - (self.menuMargin -15.0f), self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth, self.selectionIndicatorView.frame.size.height);
            }else{
                CGRect selectionIndicatorFrame = CGRectMake(selectionIndicatorX, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth, self.selectionIndicatorView.frame.size.height);
                _selectionIndicatorView.frame = selectionIndicatorFrame;
                if (_selectionIndicatorView > 0) {
                    MenuItemView *menuItemView = _mutableMenuItems[_currentPageIndex];
                    _selectionIndicatorView.centerX = menuItemView.centerX;
...
- (void)handleMenuItemTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            CGFloat menuItemRightBound = [_mutableMenuItemWidths[0] floatValue] + _menuMargin + (_menuMargin / 2) - (_menuMargin - 15.0f);
            menuItemRightBound = [_mutableMenuItemWidths[0]floatValue] + _menuMargin + 15.0f;
...
- (void)addPageAtIndex:(NSInteger)index
{
    ...
    newVC.view.frame = CGRectMake(self.view.frame.size.width * (CGFloat)index, _menuHeight - 0.5, self.view.frame.size.width, self.view.frame.size.height - _menuHeight + 0.5);
 ...
 /**     BOOL oldCurrentOrientationIsPortrait = _currentOrientationIsPortrait;
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))){
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            CGFloat selectionIndicatorX = _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth :(CGFloat)_currentPageIndex * (self.view.frame.size.width / (CGFloat)_controllerArray.count);
            CGFloat selectionIndicatorWidth = _selectionIndicatorWidth > 0 ? _selectionIndicatorWidth : self.view.frame.size.width / (CGFloat)_controllerArray.count;
            CGRect selectionIndicatorFrame = CGRectMake(selectionIndicatorX, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth, self.selectionIndicatorView.frame.size.height);
            _selectionIndicatorView.frame = selectionIndicatorFrame;
            if (_selectionIndicatorView > 0) {
                MenuItemView *menuItemView = _mutableMenuItems[_currentPageIndex];
...
//            _selectionIndicatorView.frame =  CGRectMake(selectionIndicatorX, self.selectionIndicatorView.frame.origin.y, self.selectionIndicatorView.frame.size.width, self.selectionIndicatorView.frame.size.height);
            ...
            CGRect selectionIndicatorFrame = CGRectMake(selectionIndicatorX, self.selectionIndicatorView.frame.origin.y, self.selectionIndicatorView.frame.size.width, self.selectionIndicatorView.frame.size.height);
            _selectionIndicatorView.frame = selectionIndicatorFrame;
            if (_selectionIndicatorView > 0) {
                MenuItemView *menuItemView = _mutableMenuItems[_currentPageIndex];
                _selectionIndicatorView.centerX = menuItemView.centerX;
        ...
        for (UIView *view in _controllerScrollView.subviews) {
            view.frame = CGRectMake(self.view.frame.size.width * (CGFloat)(_currentPageIndex), _menuHeight-0.5, _controllerScrollView.frame.size.width, self.view.frame.size.height - _menuHeight + 0.5);
        }
...
- (void)moveToNextPage:(NSInteger)index
{
    if (index >= 0 && index < _controllerArray.count) {
        // Update page if changed
        if (index != _currentPageIndex) {
